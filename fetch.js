 ...
 *  @param {String} options.authorization_name token名字{header: { Authorization: xxxx }}
 *  @param {String} options.base_url 请求公用前缀('https://www.xxx.com/api')
...
export default class XRequest {
	...
	// 格式化参数并添加到url上 (https://www.xxx.com?a=1&b=2)
	...
	_handleError(err, reject) {
		if (!err?.status) {
			err = {
				content: null,
				msg: err?.message || '网络请求失败，请重试！',
				status: err?.code || -10086
			}
		}
	...
	async request(url, options, config) {
		...
		const body = this._generateBodyData(method, isFormData, options)
